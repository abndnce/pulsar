...
export class NostrServerConnection
  implements PulsarServerConnection, TunnelWireTarget {
...
export class PulsarNostrServer {
  ...
  async start(): Promise<{ pubkey: string }> {
    ...
    await Promise.all(
      NOSTR_RELAYS.map((relayUrl) =>
        this.connectToRelay(relayUrl).catch((err) => {
          ...
          errors.push(error);
          console.error(
            `[nostr] Failed to connect to ${relayUrl}: ${error.message}`,
          );
        })
      ),
    );
    ...
    if (this.wsConnections.length === 0) {
      throw new Error(
        `Failed to connect to any Nostr relay: ${
          errors
            .map((err) => err.message)
            .join("; ")
        }`,
  ...
  private async publishDiscovery(): Promise<void> {
    const signed = await signNostrEvent(
      makeDiscoveryEvent(this.pubkey),
      this.seckey,
    );
  ...
  private subscribeSignaling(ws: WebSocket): void {
    ...
    ws.addEventListener("message", (event) => {
      ...
      this.handleSignalingEvent(msg[2], ws).catch((err) => {
        const error = err instanceof Error ? err : new Error(String(err));
        console.error(
          `[nostr] Error handling signaling event: ${error.message}`,
        );
        this.onerror?.(error);
      });
    });
  ...
  private async handleSignalingEvent(
    ...
    try {
      const plaintext = await decryptSignal(
        event.content,
        this.seckey,
        event.pubkey,
      );
      ...
      console.error(
        `[nostr] Failed to decrypt signaling from ${event.pubkey}: ${err}`,
      );
      return;
    }
  ...
  private async handleOffer(
    ...
    console.log(
      `[nostr] Received WebRTC offer from ${clientPubkey.slice(0, 8)}`,
    );
    ...
    pc.ondatachannel = (event) => {
      ...
      if (!sctpTransport || !dtlsTransport) {
        this.onerror?.(
          new Error("Nostr peer connection missing SCTP transport"),
        );
        return;
      }

      console.log(
        `[nostr] Keepalive channel open for ${clientPubkey.slice(0, 8)}`,
      );
      this.onconnection?.(
        new NostrServerConnection(dtlsTransport, sctpTransport, channel, pc),
      );
    };
